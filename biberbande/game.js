...
var showOutermostCards = () => {
  ...
  //for Screenreader
  var currentState = document.getElementById('GameState').innerHTML;
  setCurrentGameState(`Erste Karte ${outerCard1} Vierte Karte ${outerCard2} `);

  setTimeout(() => {
    resetOutermostCards(currentState);
  }, showOutermostTimeout);
}

var resetOutermostCards = (previousState) => {
  document.getElementById('PlayerCard0').innerHTML = "Spieler Karte 1"
  document.getElementById('PlayerCard3').innerHTML = "Spieler Karte 4"
...
var showComputerCardsAsText = (cards) => {
  for (count = 0; count < 4; count++) {
    ...
    document.getElementById('CompCard' + count).innerHTML = "Computer Karte " + (count + 1) + ":<br><br>" + cards[count]
  }
}
...
var showHumanCardsAsText = (cards) => {
  for (count = 0; count < 4; count++) {
    ...
    document.getElementById('PlayerCard' + count).innerHTML = "Spieler Karte " + (count + 1) + ":<br><br>" + cards[count]
  }
}
...
var showDefaultText = () => {
  ...
  for (count = 0; count < 4; count++) {
    document.getElementById('CompCard' + count).innerHTML = "Computer Karte " + (count + 1);
    document.getElementById('PlayerCard' + count).innerHTML = "Spieler Karte " + (count + 1);
  }
...
const play = (game) => {
  ...
  const gameOn = () => {
    ...
    var logInfo = `Du bist dran ${currentPlayer} entweder die Karte ${discardPile[discardPile.length - 1]} vom Ablagestapel tauschen oder neue Karte ziehen?`;
    ...
    switch (selectedPile) {
      case PILE.DISCARD_PILE:
        ...
        default:
          ...
          //for Screenreader
          setCurrentGameState(`Karte ${currentCard} gezogen. Tauschen oder wegwerfen?`)
          break;
    }
  };
  ...
  window.drawDiscardPile = () => draw({
    selectedPile: PILE.DISCARD_PILE
  });
  window.swap = (position) => swap({
    position
  });
